<?php
// =====================================================
// save_checklist.php - MODIFIÉ V7 (Correction Heure UTC)
// - Ajout tag [SAC DPS] dans le log
// - Correction: Forcer le sujet de l'email en [SAC DPS]
// - CORRIGÉ V4: Recherche NOM dans BDD
// - AJOUT V5: Insère le type 'DPS' dans checklists_history
// - CORRECTION V6: La requête INSERT avait 5 '?' pour 4 variables.
...
try {
    ...
    $stmt_get_name = $pdo->prepare("SELECT nom FROM produits WHERE id = ?");
    ...
    // Etape 2: Sauvegarder les items (uniquement pour DPS)
    ...
    foreach ($products_data as $produit_id => $details) {
        if (is_numeric($produit_id) && $produit_id > 0) {
            ...
            error_log("save_checklist.php: ID de produit non valide trouvé dans les données JSON: " . print_r($produit_id, true));
        }
    }

    // Etape 3: Logger l'événement
    log_event($pdo, $user, "Validation checklist [SAC DPS] #$checklist_id");
    ...
    echo json_encode([
        ...
        'message' => 'Checklist enregistrée' . ($emailSent ? ' et notifiée' : ''),
        'status' => $status,
        'email_sent' => $emailSent
    ]);

} catch (PDOException $e) {
    error_log("Erreur save_checklist: " . $e->getMessage());
    echo json_encode([
        'success' => false,
        'message' => 'Erreur BDD: ' . $e->getMessage()
    ]);
} catch (Exception $e) {
     error_log("Erreur Générale save_checklist: " . $e->getMessage());
     echo json_encode([
        'success' => false,
    ...
    function sendChecklistEmail($pdo, $checklist_id, $user, $status_fr, $items_manquants, $items_defaillants, $commentaire) {
        ...
        try {
            ...
            if ($condition === 'issues_only' && !$has_issues) {
                return true; // Pas d'envoi requis, succès implicite
            }
             if (empty($destinataire)) {
                 log_event($pdo, 'Système', "Email non envoyé (checklist #$checklist_id): Aucun destinataire configuré.");
                 return false; // Pas de destinataire configuré
            }


            // 3. Préparer les variables
            $now = new DateTime('now', new DateTimeZone('Europe/Paris')); // Utiliser le fuseau horaire correct
            $date_fr = format_date_fr($now->format('Y-m-d H:i:s')); // Formate la date actuelle
            ...
            // 5. Préparer l'envoi
            $prefix = "🚒 [SAC DPS]"; // Forcé pour DPS
            $sujet = "$prefix Checklist #$checklist_id ($status_fr)";
             if ($has_issues) {
                $sujet = "$prefix ⚠️ Checklist #$checklist_id ($status_fr)";
            }
            $expediteur_name = "Checklist DPS"; // Nom spécifique

            $expediteur_email = 'noreply@' . ($_SERVER['HTTP_HOST'] ?? 'votre-domaine.com'); // Mettre un domaine par défaut
            ...
            error_log("Erreur sendChecklistEmail: " . $e->getMessage());
