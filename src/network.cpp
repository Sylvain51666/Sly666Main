...
static void handlePoolTopics(const char* payloadCStr) {
    ...
    // Extraction température piscine
...
// ============================== CORRECTION #3: BUFFER OVERFLOW MQTT ==============================
static void mqttCallback(char* topic, byte* payload, unsigned int length) {
    sysState.lastDataReceived = millis();
    
    // CORRECTION #3: Limite de taille pour éviter stack overflow
    constexpr unsigned int MAX_PAYLOAD_SIZE = 512;
    
    if (length > MAX_PAYLOAD_SIZE) {
        DataLogging::writeLog(LogLevel::LOG_WARN, 
            "[Network] MQTT payload too large (" + String(length) + " bytes), truncating to " + String(MAX_PAYLOAD_SIZE));
        length = MAX_PAYLOAD_SIZE;
    }
    
    // Buffer de taille fixe et sécurisée
    char payloadStr[MAX_PAYLOAD_SIZE + 1];
    memcpy(payloadStr, payload, length);
    payloadStr[length] = '\0';
    
    String topicStr = String(topic);
    
    // Traitement des topics de facturation
    if (topicStr == SETTINGS.topic_sub_price || 
        topicStr == SETTINGS.topic_hc_price || 
        topicStr == SETTINGS.topic_bill_day || 
        topicStr == SETTINGS.topic_hp_price) {
        InvoicesStore::handleMqttMessage(topicStr, String(payloadStr));
    }
    // Traitement piscine
    else if (topicStr == SETTINGS.topic_piscine) { 
        handlePoolTopics(payloadStr); 
    }
    // Traitement eau
    else if (topicStr == SETTINGS.topic_eau || topicStr == SETTINGS.topic_talon_eau) { 
        handleWaterTopics(topic, payloadStr); 
    }
    // Traitement heure et pompe
    else if (topicStr == SETTINGS.topic_heure || 
             topicStr == SETTINGS.topic_pompe_hr_start || 
             topicStr == SETTINGS.topic_pompe_hr_end) { 
        handleTimeAndPumpSchedule(topic, payloadStr); 
    }
    // Traitement pompe piscine
    else if (topicStr == SETTINGS.topic_pompe_piscine) {
        bool newS = (strcasecmp(payloadStr, "true") == 0 || 
                     strcmp(payloadStr, "1") == 0 || 
                     strcasecmp(payloadStr, "on") == 0);
        if (newS != sensorData.isPumpRunning) {
            sensorData.isPumpRunning = newS;
            DataLogging::writeLog(LogLevel::LOG_INFO, 
                String("Pompe piscine: ") + (sensorData.isPumpRunning ? "ON" : "OFF"));
        }
    }
    // Traitement solaire
    else if (topicStr == SETTINGS.topic_autoconsommation || 
             topicStr == SETTINGS.topic_prod_totale || 
             topicStr == SETTINGS.topic_temp_onduleur_1 || 
             topicStr == SETTINGS.topic_temp_onduleur_2) {
        handleSolarTopics(topic, payloadStr);
    }
    // Traitement humidité
    else if (topicStr == SETTINGS.topic_studio_humidity) { 
        handleHumidityTopic(payloadStr); 
    }
}
