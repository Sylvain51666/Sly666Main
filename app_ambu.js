...
const DEFAULT_CONFIG = {
    ...
    storageKey: 'checklistAMBU_draft', // Clé différente
    ...
    text_ambu_title: "Checklist Ambulance",
    text_ambu_welcomeMessage: "Bienvenue dans la checklist Ambulance.<br>Vérifiez chaque zone et validez votre contrôle.",
};

const AMBU_CONFIG = {};
if (window.DYNAMIC_CONFIG) {
    Object.keys(window.DYNAMIC_CONFIG).forEach(key => {
        const newKey = key.startsWith('ambu_') ? key.substring(5) : key;
        AMBU_CONFIG[newKey] = window.DYNAMIC_CONFIG[key];
    });
}
...
// Needs a specific key like 'app_ambu_draftMaxAgeMinutes' or reuse 'app_draftMaxAgeMinutes'
...
document.addEventListener('DOMContentLoaded', function() {
    console.log('🚀 CheckList AMBU - Initialisation (Logique V10.2)');
    console.log('Config chargée:', CONFIG);

    initThemeToggle();
    initViewModeToggle();
    initModal();
    initProducts(); // IMPORTANT: Popule checklistState avec les NOMS
    initCommentaire();
    initSendButton();
    initWelcomePopup();
    initAutoSave();
    initResetModal();
    restoreDraft(); // IMPORTANT: Doit venir APRES initProducts
...
function updateModalStaticTexts() {
     ...
     // Utilise les textes de reset spécifiques AMBU s'ils existent, sinon les génériques
     if (titleElement) titleElement.textContent = CONFIG.text_resetConfirmTitle || CONFIG.text_resetModalTitle;
     if (messageElement) messageElement.textContent = CONFIG.text_resetConfirmMessage || CONFIG.text_resetModalMessage;
     if (confirmBtn) confirmBtn.textContent = CONFIG.text_resetConfirmYes || CONFIG.text_resetModalConfirmButton;
     if (cancelBtn) cancelBtn.textContent = CONFIG.text_resetConfirmNo || CONFIG.text_resetModalCancelButton;
}
...
function createStatusButtons(product, select) {
    ...
    const statuses = [
        { value: 'vide', label: CONFIG.text_ambu_okLabel || 'OK', icon: '✅' },
        { value: 'manquant', label: CONFIG.text_ambu_missingLabel || 'Manquant', icon: '❌' },
        { value: 'defaillant', label: CONFIG.text_ambu_defectiveLabel || 'Défaillant', icon: '⚠️' }
    ];
...
function sendChecklistToServer(missingProducts, defaillantProducts) {
    ...
    // Appel Fetch (ATTENTION: pointe vers le bon fichier PHP)
    fetch('save_checklist_ambu.php', {
        method: 'POST',
...
function initWelcomePopup() {
    ...
    // Utilise les textes spécifiques AMBU
    const title = CONFIG.text_ambu_welcomeTitle || CONFIG.text_welcomeTitle;
    const message = CONFIG.text_ambu_welcomeMessage || CONFIG.text_welcomeMessage;

    if (popup && titleElement && messageElement) {
        // Mettre à jour le contenu avec les textes de CONFIG
        titleElement.innerHTML = title;
        messageElement.innerHTML = message;

        // Afficher le popup (devrait déjà être fait via CSS si la classe 'show' est présente par défaut)
        // Mais on s'assure qu'il est visible au cas où
...
function saveDraft() {
    try {
        ...
        // console.log("💾 Brouillon AMBU sauvegardé:", new Date(draft.lastSaved));
    } catch (e) {
        console.error('Erreur sauvegarde brouillon AMBU:', e);
...
function restoreDraft() {
    try {
        ...
        if (draftJSON) {
            ...
            if (draft.lastSaved && CONFIG.draftMaxAgeMinutes > 0) {
                ...
                if (ageInMinutes > CONFIG.draftMaxAgeMinutes) {
                    console.log(`🗑️ Brouillon AMBU expiré (plus de ${CONFIG.draftMaxAgeMinutes} minutes). Suppression.`);
                    clearDraft();
                ...
                if (product) {
                    ...
                    if (checklistState.products[productId]) {
                        ...
                        // 1. Mettre à jour l'état global (fusionner)
                         if (productStateFromDraft.etat !== undefined) { // Check existence
                        ...
                        if(select && checkbox) {
                         ...
                         console.warn(`Produit AMBU ID ${productId} trouvé dans le brouillon mais pas dans la page. Ignoré.`);
                    }
                }
            });
        ...
        console.error('Erreur restauration brouillon AMBU:', e);
...
function clearDraft() {
    try {
        ...
        console.log("🗑️ Brouillon AMBU effacé.");
    } catch (e) {
        console.error('Erreur suppression brouillon AMBU:', e);
    }
}
